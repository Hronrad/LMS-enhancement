...
最新功能 v 0.50：经同学反应原加速逻辑已失效，重新对 LMS 改用播放器原生播放按钮并验证视频时间是否真实推进；增加后台播放保护、同源 iframe/Shadow DOM 支持、连续观看范围检查和结束后延迟跳转；统一认证登录页支持延时自动登录及拼图滑块自动处理。保留多档倍速、虚拟观看会话、课件下载和 MOOC 功能。

近期更新历史 
- v 0.50：重构 LMS 原生播放、后台恢复和章节切换逻辑；移除 OCR 服务，改为统一认证延时自动登录及拼图滑块处理；视频辅助默认开启，并补充 0.5x、1.5x、2x 倍速。
